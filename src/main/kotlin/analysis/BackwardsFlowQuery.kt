...
/**
 * Queries whether a symbol at some location satisfies some condition, the result
 * of this check being represented by [T].
 *
 * Only concrete implementers are the matchers generated by [PatternMatcher].
 */
interface SymbolQuery<T> {
    /**
     * Checks whether the definition of [q] reachable from [src] matches some pattern, the result
     * of this match being expressed in [T].
     *
     * NB that if [src] is actually `q := E` and `E` matches the pattern, this is *not* considered a match,
     * as we are looking for matches starting in the RHS of [src].
     */
    fun query(q: TACSymbol.Var, src: LTACCmd) : T

    /**
     * Checks whether the variable defined at [start] matches the pattern represented by this class. In other words,
     * this is a version of [query] which starts from the LHS.
     */
    fun queryFrom(start: LTACCmdView<TACCmd.Simple.AssigningCmd>) : T
}

/**
 * A [SymbolQuery] that always returns the same [result] for any query
 */
class ConstantQuery<T>(val result: T) : SymbolQuery<T> {
    override fun query(q: TACSymbol.Var, src: LTACCmd): T = result

    override fun queryFrom(start: LTACCmdView<TACCmd.Simple.AssigningCmd>): T = result
